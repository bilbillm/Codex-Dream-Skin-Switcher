...
Windows 版 Codex 自定义主题预览、导入、热切换与一键启动工具。当前 `v0.1.0` 内置 Angelina Gravity Field 白天主题和 Angelina Midnight Gravity 黑夜主题，并把 Dream Skin 的脚本式工作流包装成一个安静、可检查的 Windows GUI。
...
- `v0.1.0` 自包含包固定使用 .NET Windows Desktop Runtime `10.0.10`。
...
1. 打开开始菜单中的“Codex 主题切换器”或运行 `CodexThemeSwitcher.exe`。
2. 在左侧主题库选择主题。
...
```mermaid
flowchart LR
    A["CodexThemeSwitcher.exe"] -->|"读取"| B["themes/*/theme.json"]
...
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File .\scripts\verify-release.ps1 -Version 0.1.0
```
...
```text
artifacts/
├─ Codex-Dream-Skin-Switcher-v0.1.0-win-x64/
├─ Codex-Dream-Skin-Switcher-v0.1.0-win-x64.zip
├─ publish/
└─ SHA256SUMS.txt
```
...
```powershell
.\scripts\build-release.ps1 -Version 0.1.0 -FrameworkDependent
```
...
Release 中是同一个 `CodexThemeSwitcher.exe`。无参数启动时打开主题切换器，带 `--launch` 时进入一键启动模式；安装脚本用快捷方式保存这个参数。
