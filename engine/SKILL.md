...
1. Install Node.js 22 or newer, close ChatGPT/Codex, then run `scripts/install-dream-skin.ps1` once. The installer preserves the user's native appearance settings, seeds bright Angelina as the default plus Angelina Midnight Gravity and the upstream Arina Hashimoto and Gothic presets, copies the runtime to `%LOCALAPPDATA%\CodexDreamSkin\engine`, and creates source-independent launch/restore/tray shortcuts.
2. Use the `Codex Angelina Gravity Field` shortcut, or run `%LOCALAPPDATA%\CodexDreamSkin\engine\scripts\start-dream-skin.ps1`. The shortcut asks before restarting an already-open ChatGPT/Codex app; CLI callers must explicitly add `-RestartExisting`.
...
- `assets/angelina-hero.png`: 2048 × 1152 Angelina home visual seeded as the default.
- `assets/angelina-thread-bg.jpg`: 1280 × 720 pre-blurred task background persisted through the optional `taskImage` theme field.
- `presets/preset-angelina-midnight-gravity/`: dark Angelina preset with an independent 2048 × 1152 home image and 1280 × 720 task image.
- `assets/dream-reference.jpg`: upstream 2560 × 1440 Arina Hashimoto wallpaper, retained as a saved preset and image-metadata regression fixture.
